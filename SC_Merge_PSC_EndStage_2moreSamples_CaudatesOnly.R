...
dir="/cluster/projects/macparland/TA/PSC/Analysis_vs_20LiverMap/"

##### Input #####
PSC_sc_5pr = c(
	"/cluster/projects/macparland/TA/PSC/Processed/PSC018_5pr_caudate_SoupX.rds",
	"/cluster/projects/macparland/TA/PSC/Processed/PSC014X_5pr_caudate_SoupX.rds",
	"/cluster/projects/macparland/TA/PSC/Processed/PSC019_Caudate_5pr_SoupX.rds",
	"/cluster/projects/macparland/TA/PSC/Processed/PSC024_Caudate_5pr_SoupX.rds"
	)

outname <- "SC_Map/PSC_SC_5pr_4by4_noSec3";

sample_names <- strsplit(PSC_sc_5pr, "/");
sample_names <- sapply(sample_names, function(x){x[length(x)]})
sample_names <- sub("_SoupX.rds", "", sample_names)
...
for (i in 1:length(PSC_sc_5pr)) {
	n <- sample_names[i];
	print(n);
	donor_id <- sapply(strsplit(n, "_"), function(x){x[1]})
	obj <- readRDS(PSC_sc_5pr[i])
	print(dim(obj))
	obj <- obj[,
	...
	obj@meta.data$Phenotype <- "PSC"
	if (grepl(PSC_sc_5pr[i], "_5pr_")) {
		obj@meta.data$assay_type <- "5pr"
	}
	if (grepl(PSC_sc_5pr[i], "_PBMCs_")) {
		obj@meta.data$sample_type <- "PBMCs"
	}
	if (grepl(PSC_sc_5pr[i], "_Section3_")) {
		obj@meta.data$sample_type <- "Section3"
	}
	if (grepl(PSC_sc_5pr[i], "_biopsy_")) {
		obj@meta.data$sample_type <- "biopsy"
	}
