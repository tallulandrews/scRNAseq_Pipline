get_cluster_summary <- function(obj, clusters="Subcluster_Manual", samples="donor", presence_t = 5) {
	tab <- table(obj@meta.data[,clusters], obj@meta.data[,samples])
	tot <- rowSums(tab)
	simpson <- rowSums(tab^2)/tot^2
	nsamples <- rowSums(tab > presence_t)

	out_tab <- tab
	out_tab <- cbind(out_tab, tot)
	out_tab <- cbind(out_tab, simpson)
	out_tab <- cbind(out_tab, nsamples)
	return(out_tab)
}
